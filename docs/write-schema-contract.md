...
Version 0.9.0 exposes 21 public GitHub write tools (64 total public MCP tools: 43 read-only and 21 write) through one canonical current host-facing registry. This document defines their schema, metadata, registration, authorization, and ambiguity invariants. It does not weaken execution, exact-state, mutation-attempt, or readback requirements.
...
- `tests/test_release_gate_0_9_0.py` pins package/lock/runtime versions, exact 64/43/21 inventory, retired-tool absence, single canonical write registration, compatibility-path removal, default-off fine gates, current static authority, and current documentation;
- `tests/test_tool_schema_snapshot.py` pins the complete current 64-tool schema surface and uses explicit output-schema narrowing for changed-test Pyrefly validation;
...
- `tests/test_write_transport_metadata.py` pins structured ambiguity authority, rejection of bare-`RuntimeError` text inference, and the current 64/43/21 documentation inventory.
