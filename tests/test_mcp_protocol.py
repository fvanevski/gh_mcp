...
async def test_stdio_write_denial_does_not_elicit_or_lock_session() -> None:
    ...
    async with (
        ...
        assert len(tools_after_failure.tools) == 59
...
async def test_stdio_write_executes_once_without_elicitation_using_fake_gh(tmp_path: Path) -> None:
    ...
    async with (
        ...
        assert len((await session.list_tools()).tools) == 59
...
async def test_streamable_http_content_route_keeps_namespace_live(
    ...
    try:
        async with (
            ...
            assert initialized.server_info.version == "0.7.1"
            ...
            assert server_info_after_denial.structured_content["server_version"] == "0.7.1"
