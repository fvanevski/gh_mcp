...
from mcp_gh_server.merge_requirements_models import MergeRequirementEvidenceSource
...
from mcp_gh_server.server import AppContext, mcp
...
async def test_public_output_schema_exposes_bounded_evidence_sources() -> None:
    ...
    assert properties["source"]["maxLength"] == 80
    ...
    reason_schema = next(
        item for item in properties["reason"]["anyOf"] if item.get("type") == "string"
    )
    assert reason_schema["maxLength"] == 512
    assert properties["blocks_policy_evidence"]["type"] == "boolean"
    assert properties["blocks_checks_evidence"]["type"] == "boolean"
    assert properties["blocks_allowed_merge_methods"]["type"] == "boolean"
...
async def test_classic_404_with_verified_admin_read_is_authoritative_absence() -> None:
    ...
    assert any(
        args[:2] == ("api", "repos/octo/repo/rulesets/rule-suites")
...
async def test_active_rules_compose_when_classic_404_is_verified_absent() -> None:
    ...
    assert {
        ...
        for check in result.policy.required_status_checks.values()
    } == {("Pyrefly", None)}
...
async def test_firecrawl_285_shape_becomes_complete_only_with_all_policy_sources(
    monkeypatch: Any,
) -> None:
    base = "a" * 40
    head = "b" * 40
    ...
    assert [check.name for check in result.current_required_checks] == ["Pyrefly"]
    assert result.allowed_merge_methods == ["merge", "squash", "rebase"]
    assert result.warning is None

    classic = _source(result, "classic_branch_protection")
    permission = _source(result, "classic_protection_admin_permission")
    checks = _source(result, "current_required_checks")
    identity = _source(result, "pull_request_identity")
    assert (classic.status, classic.http_status) == ("absent", 404)
    assert permission.status == "complete"
    assert checks.status == "complete"
    assert identity.status == "complete"
    assert not any(
        source.blocks_policy_evidence
        or source.blocks_checks_evidence
        or source.blocks_allowed_merge_methods
        for source in result.evidence_sources
    )


async def test_firecrawl_285_shape_stays_fail_closed_when_admin_probe_is_forbidden(
    monkeypatch: Any,
) -> None:
    base = "a" * 40
    head = "b" * 40
